...
## 1. Identity
- Module / area: transaction
- Page type: Report
- Primary users / roles: Authenticated tenant users with menu grant; Admin/Super Admin unrestricted for administration screens
- Entry points: Menu **Stock Journal Report**; deep links from related registers/dashboards where applicable
- Priority for rebuild: P1
- Wave: 2
- Menu ID: 428

## 2. Purpose
Accounting voucher / money document screen. Must post double-entry ledger lines, support numbering series, cancel/unpost, and (for payment/receipt) bill-wise ageing allocation where applicable. See also [transaction/vouchers.md](../transaction/vouchers.md) for shared voucher engine rules.
...
- Entities involved: Voucher, VoucherTrans, AccLedger, AccLedgerTrans, AccAgeing
...
- Given a user with menu access, they can open **Stock Journal Report** and see tenant-scoped data
...
- Old URL: `reports/ItemTransaction/stock_journal`
